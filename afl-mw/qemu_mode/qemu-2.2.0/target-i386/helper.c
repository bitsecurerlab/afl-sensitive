...
#include "../linux-user/rb_tree.h"
 // extern void write_bit_map(uint32_t, uint32_t);

static uint32_t mem_access_hash = 0;


uint32_t get_mem_access()
{
    return mem_access_hash;
}
...
// void helper_log_mem_access(target_ulong pc, target_ulong addr, int r_w)
// {
//     struct heap_node* hn = heap_obj_search(&heap_object_tree, addr);
//     if(hn)
//     {
//         addr = hn->ip;
//     }
//     if(r_w == 0)
//     {
//         mem_access_hash = (( HALF_MAP_SIZE -1) & addr); // + mem_access_hash * 3; 
//     }
//     else
//     {
//         mem_access_hash =  HALF_MAP_SIZE + (( HALF_MAP_SIZE -1) & addr); // + mem_access_hash * 3;  
//     }
// }

void helper_log_mem_access(target_ulong pc, target_ulong addr)
{
    struct heap_node* hn = heap_obj_search(&heap_object_tree, addr);
    if(hn)
    {
    ...
    mem_access_hash =  ((MAP_SIZE -1) & addr); // + mem_access_hash * 3;  
...
{
    afl_maybe_log(pc, from, get_mem_access());
}

...
static void write_stats_file(double bitmap_cvg, double eps) {
  ...
  fprintf(f, "start_time                  : %llu\n"
             ...
             "checked_paths_fuz           : %i\n"
             "imported_paths_driller      : %i\n"
             "checked_paths_driller       : %i\n"
             ...
             imported_paths[DRL_SRC], checked_paths[DRL_SRC],
...
static void sync_fuzzers(char** argv) {
  ...
  while ((sd_ent = readdir(sd))) {
    ...
    {
      if(!startswith(sd_ent->d_name, "driller") && !endswith(sd_ent->d_name, "filter") && !startswith(sd_ent->d_name, "traffic")) continue;
    }
    /* Skip anything that doesn't have a queue/ subdirectory. */

    if(startswith(sd_ent->d_name, "driller"))
    {
      from_which = DRL_SRC;
    }
...
int main(int argc, char** argv) {
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:t:T:dnCB:S:M:x:QLs:l:")) > 0) 
  {
    // ACTF("opt: %c", opt);
    switch (opt) {

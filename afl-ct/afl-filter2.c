...
static void write_stats_file(double bitmap_cvg, double eps) {
  ...
  fprintf(f, "start_time     : %llu\n"
             ...
             queued_imported, sync_count, sync_times,
...
int test_one_case(char *path, char** argv)
{
  int fd;
  struct stat st;
  int interesting = 0;
  ...
  if (st.st_size && st.st_size <= MAX_FILE) {
    ...
    fault = run_target(argv);
      ...
      // syncing_party = sd_ent->d_name;
    interesting = save_if_interesting(argv, mem, st.st_size, fault);
    queued_imported += interesting;
      // syncing_party = 0;
...
void filter_server(char** argv)
{
  ...
  {
    // read request
    if(sizeof(path_len) != read(FORKSRV_FD + 10, &path_len, sizeof(path_len)))
    {
      fprintf(stderr, "ERROR READ\n");
      exit(-1);
    }
    assert(path_len <= PATH_MAX+1);
    if(path_len != read(FORKSRV_FD + 10, path, path_len))
    {
      fprintf(stderr, "ERROR READ\n");
      exit(-1);
    }
    ...
    // test
    interesting = test_one_case(path, argv);
    // interesting = 0;
    fprintf(stderr, "interesting: %d\n", interesting);

    // write response
    if(sizeof(interesting)!=write(FORKSRV_FD + 10, &interesting, sizeof(interesting)))
    {
      fprintf(stderr, "WRITE RESP ERROR\n");
      exit(-1);
    }

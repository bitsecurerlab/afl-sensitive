...
// static inline void afl_maybe_log(abi_ulong, abi_ulong, abi_ulong);
...
// extern uint32_t get_calling_context();

inline uint32_t bitmap_hash(uint32_t pc, uint32_t calling_context)
{
  uint32_t index = (MAP_SIZE - 1) & (pc ^ calling_context);
  // fprintf(stderr, "index: %08x [pc: %08x, cc: %08x]\n", index, pc, calling_context);
  return index;
}
/*************************
...
//   uint32_t index = bitmap_hash(cur_loc ^ prev_loc, get_calling_context());
//   if (afl_area_ptr)
//     afl_area_ptr[index]++;
...
// static inline void afl_maybe_log(abi_ulong next_pc, abi_ulong cur_loc, abi_ulong cc) {
void afl_maybe_log(abi_ulong next_pc, abi_ulong cur_loc, abi_ulong cc) {

  // fprintf(stderr, "afl_maybe_log(0x%x, 0x%x, 0x%x) \n", next_pc, cur_loc, cc );
  ...
  uint32_t index = bitmap_hash((cur_loc >> 1) ^ next_pc, cc);
  // fprintf(stderr, "afl_maybe_log.index: 0x%x\n", index);

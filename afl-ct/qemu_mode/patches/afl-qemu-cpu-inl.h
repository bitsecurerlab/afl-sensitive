...
inline void write_bit_map(uint32_t, uint32_t);
...
// inline void write_bit_map(uint32_t pc, uint32_t memaddr)
// {
//   // fprintf(stderr, "[@]write_bit_map(%x %x)\n", pc, memaddr);
...
{
  return (MAP_SIZE - 1) & (pc ^ calling_context);
}
...
static inline uint32_t hash32(const uint32_t* key, uint32_t len, uint32_t seed) {
...
//  afl_area_ptr[cur_loc ^ prev_loc]++;
  unsigned int index = bitmap_hash((cur_loc >> 1) ^ next_pc, cc); 

...
static void cull_queue(void) {
  ...
  for (i = 0; i < MAP_SIZE * num_of_cbs; i++)
  {
    // if(top_rated[i])
    // {
    //   printf("top_rated[%d]: %s, tc_ref: %d\n", i, top_rated[i]->fname, top_rated[i]->tc_ref);
    //   printf("temp_v[%d >> 3]: %d\n", i, temp_v[i >> 3]);
    ...
    if (top_rated[i] && (temp_v[i >> 3] & (1 << (i & 7)))) {
      ...
      // printf("favored top_rated[%d]: %s, tc_ref: %d\n", i, top_rated[i]->fname, top_rated[i]->tc_ref);
...
int main(int argc, char** argv) {
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:t:T:dnCB:S:M:QLs:r")) > 0)
  {
    // ACTF("opt: %c", opt);
    switch (opt) {

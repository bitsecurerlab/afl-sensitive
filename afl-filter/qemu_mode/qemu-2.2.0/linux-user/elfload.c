                                 ...
                                 struct image_info *info)
{
    ...
    //*CGC* start: 0 -> 0xba2aa000, to be compatible with decree 
    // + PROT_EXEC
    error = target_mmap(0xba2aa000, size + guard, PROT_READ | PROT_WRITE | PROT_EXEC,
    ...
    if(info->stack_limit != 0xba2ab000)
        fprintf(stderr, "unintended stack_limit: 0x%x\n", info->stack_limit);
    if(stack_base != 0xbaaab000)
        fprintf(stderr, "unintended stack_base: 0x%x\n", stack_base);

    /* setup up flag page for CGC*/
    abi_ulong flag_page_start = 0x4347C000;
    error = target_mmap(flag_page_start, TARGET_PAGE_SIZE, PROT_READ|PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    if(error != flag_page_start)
    {
        perror("mmap flag page");
        exit(-1);
    }
    // uint8_t flag_page[TARGET_PAGE_SIZE];
    // for(i = 0; i < TARGET_PAGE_SIZE; i++)
    // {
    //     flag_page[i] = 0x30 + i % 10;
    // }
    memcpy_to_target(flag_page_start, g_flag_page, TARGET_PAGE_SIZE);
    target_mprotect(flag_page_start, TARGET_PAGE_SIZE, PROT_READ);
    return p;
}
...
{
    return load_elf_binary(bprm, info);
}

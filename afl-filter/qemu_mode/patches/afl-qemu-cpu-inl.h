...
#define AFL_QEMU_CPU_SNIPPET2 do { \
    ...
    afl_maybe_log(tb->pc, tb->pc + tb->size); \
  } while (0)
...
static void afl_forkserver(CPUArchState *env) {
  ...
  while (1) {
    ...
    if(fcntl(CODE_BLOCK_INFO_FD, F_GETFD) != -1 || errno != EBADF)
    {
      ftruncate(CODE_BLOCK_INFO_FD, 0);
      lseek(CODE_BLOCK_INFO_FD, 0, SEEK_SET);
    }
...
static inline void afl_maybe_log(abi_ulong cur_loc, abi_ulong end) {

...
static void mark_as_favored(struct queue_entry* q, u8 state)
{
  ...
  fn = strrchr(q->fname, '/');
  fn = alloc_printf("%s/queue/.state/favored_edges/%s", out_dir, fn + 1);
...
static u32 minimize_case(char** argv, u8* in_data, u32 orig_len, u32 orig_cksum) {
...
static u8 save_if_interesting(char** argv, void* mem, u32 len, u8 fault) {
  ...
  if (fault == crash_mode) {
    ...
    len = minimize_case(argv, mem, len, key_cksum);
...
static void maybe_delete_out_dir(void) {
  ...
  fn = alloc_printf("%s/.cur_code_block_info", local_out_dir);
  if (unlink(fn) && errno != ENOENT) goto dir_cleanup_failed;
  ck_free(fn);
...
static void sync_fuzzers(char** argv) {
  ...
  while ((sd_ent = readdir(sd))) {
    ...
    for(i=0; i<2; i++)
    {
      ...
      qd_synced_path = alloc_printf("%s/.synced/%s_%s", local_out_dir, sd_ent->d_name, q_or_c[i]);
...
static void setup_stdio_file(void) {
  ...
  /* cgc setup fd for communicating covered code block info */
...
int main(int argc, char** argv) {
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:t:T:dnCB:S:M:QLs:")) > 0)
  {
    // ACTF("opt: %c", opt);
    switch (opt) {
  ...
  if (sync_id)
  {
    ck_free(local_out_dir);
    local_out_dir = alloc_printf("/home/vagrant/local_outputs/%s", sync_id);
    ck_free(remote_out_dir);
    remote_out_dir = alloc_printf("/home/vagrant/shared_outputs/%s", sync_id);
  }

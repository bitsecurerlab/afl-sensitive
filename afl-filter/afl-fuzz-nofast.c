...
static void mark_as_favored(struct queue_entry* q, u8 state)
{
  ...
  fn = strrchr(q->fname, '/');
  fn = alloc_printf("%s/favor_queue/%s", out_dir, fn + 1);
...
static void maybe_delete_out_dir(void) {
  ...
  fn = alloc_printf("%s/favor_queue", out_dir);
  if(delete_files(fn, CASE_PREFIX)) goto dir_cleanup_failed;
  ck_free(fn);
...
static void sync_fuzzers(char** argv) {
  ...
  while ((sd_ent = readdir(sd))) {
    ...
    { 
      char s2e[] = "s2e";
      if(!strcmp(s2e, sd_ent->d_name)) continue;
    }
...
static void setup_dirs_fds(void) {
  ...
  tmp = alloc_printf("%s/favor_queue", out_dir);
  if (mkdir(tmp, 0700)) PFATAL("Unable to create '%s'", tmp);
  ck_free(tmp);

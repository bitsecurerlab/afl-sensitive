...
//global maintained heap objects through rb tree
struct rb_root heap_object_tree;

void display_hot_iter(struct rb_node *tnode, int depth)
{
    if(tnode)
    {
        struct heap_node *data = rb_entry(tnode, struct heap_node, node);
        fprintf(stderr, "#%d:[0x%x, 0x%x) color=%d\n", depth, data->start, data->end, rb_color(tnode));
        fprintf(stderr, "left_tree:  ");
        display_hot_iter(tnode->rb_left, depth+1);
        fprintf(stderr, "right_tree:  ");
        display_hot_iter(tnode->rb_right,depth+1);
    }
    else
    {
        fprintf(stderr, "#%d:N/A\n", depth);
    }   
}


//print out the rb tree
void display_hot(struct rb_root *root)
{
    fprintf(stderr, "display_hot()\n");
    struct rb_node *node = root->rb_node;
    display_hot_iter(node, 0);
}

int heap_obj_insert(struct rb_root *root, struct heap_node *data)
{
    struct rb_node **new = &(root->rb_node), *parent = NULL;
    while(*new)
    {
        struct heap_node *this = rb_entry(*new, struct heap_node, node);

        parent = *new;
        if (data->start >= this->end)
        {
            new = &((*new)->rb_right);
        }
        else if(data->end <= this->start)
        {
            new = &((*new)->rb_left);
        }
        else
...
struct heap_node* heap_obj_search(struct rb_root* root, uint32_t addr)
{
...
int heap_obj_free(struct rb_root* root, uint32_t addr)
{
    // fprintf(stderr, "heap_obj_free(0x%x)\n", addr);
    struct heap_node *free_data = heap_obj_search(root, addr);
    if(free_data)
    {
        // display_hot(root);
        rb_erase(&free_data->node, root);
        free(free_data);
        free_data = NULL;
        return 0;
    }
    ...
    switch(num) {
      ...
      case TARGET_NR_allocate://cgcos_allocate(unsigned long len, unsigned long exec, void __user **addr)
      {
        ...
        else
        {
            ...
            // fprintf(stderr, "\nallocate(len=%d,...) = 0x%x\n", arg1, ret);
            // struct heap_node *new_heap_obj = malloc(sizeof(struct heap_node));
            // new_heap_obj->start = ret;
            // new_heap_obj->end = ret + (uint32_t)arg1;
            // new_heap_obj->ip = ((X86_CPU(cpu))->env).eip;
            // heap_obj_insert(&heap_object_tree, new_heap_obj);
